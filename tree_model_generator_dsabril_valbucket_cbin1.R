...
#Variables iniciales
path <-  "~/Documents/master/datamining_economia/R_projects/models/basic_treem_cbin1/"
model_name <- "treem_dsapril_cbin1_"
...
split_values <- c(20,50,100,200,500)
...
rs = dbSendQuery(mydb, "
              SELECT\ 
                 ...
                 participa,\
                 clase
            FROM cliente_premium_abril_cbin1;")
...
cat( "fecha", "archivo", "algoritmo", "cp" , "minsplit", "minbucket", "maxdepth", "ganancia_promedio", "tiempo_promedio", "ganancias" , "\n", sep="\t", 
        file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_varminbacket_cbin1", fill=FALSE, append=FALSE )
...
for( vminsplit  in  split_values )
{
  for(vminbucket in minbucket_values)
  {
    ...
    for( s in  1:5 )
    {
      ...
      #guardamos el modelo
      vcp_and_seed_str <-paste("CP_", as.character(vcp),"-SEED_", as.character(s),"-SPLIT_", as.character(vminsplit),"-BUCKET_", as.character(vminbucket),"-CB1",  sep = '')
      ...
      cat( format(Sys.time(), "%Y%m%d %H%M%S"), "prodprem_201604", "rpart",
           vcp , vminsplit, vminbucket, vmaxdepth, mean(ganancias), mean(tiempos), ganancias , "\n", sep="\t", file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_varminbacket_cbin1", fill=FALSE, append=TRUE )
