...
cat( "fecha", "archivo", "algoritmo", "cp" , "minsplit", "minbucket", "maxdepth", "ganancia_promedio", "tiempo_promedio", "ganancias" , "\n", sep="\t", 
        file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_vardepth_cbin2", fill=FALSE, append=FALSE )
...
for( vmaxdepth  in  depth_values )
{
  ...
  for( s in  1:5 )
  {
    ...
    #guardamos el modelo
    vcp_and_seed_str <-paste("CP_", as.character(vcp),"-SEED_", as.character(s),"-DEPTH_", as.character(vmaxdepth),"-CB2", sep = '')
    ...
    cat( format(Sys.time(), "%Y%m%d %H%M%S"), "prodprem_201604", "rpart",
       vcp , vminsplit, vminbucket, vmaxdepth, mean(ganancias), mean(tiempos), ganancias , "\n", sep="\t", file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_vardepth_cbin2", fill=FALSE, append=TRUE )
