...
#Variables iniciales
path <- "~/Documents/master/datamining_economia/R_projects/models/basic_treem/"
model_name <- "treem_dsapril_"

#Mejores valores elejidos de corridas anteriores
seed_values <- c( 17823843, 0456963, 98643567, 5565754, 0364385962)
split_values <- c(20,50,60)
minbucket_values <- c(2,3)
cp_values <- c(0, 0.0001, 0.005)
depth_values <- c(9,10,12,20)
...
cat( "fecha", "archivo", "algoritmo", "cp" , "minsplit", "minbucket", "maxdepth", "ganancia_promedio", "tiempo_promedio", "ganancias" , "\n", sep="\t", 
        file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_bestparams", fill=FALSE, append=FALSE )
for ( vcp  in  cp_values ) 
{
  
  for ( vmaxdepth  in  depth_values ) 
  {
    for( vminsplit  in  split_values )
    {
      for(vminbucket in minbucket_values)
      {
        ganancias <- c() 
        tiempos <- c()
        ...
        for( s in  1:5 )
        {
          ...
          t0 =  Sys.time()
          tree_model  <- rpart( clase ~ .   ,   data = ds_cliente_premium_training, cp=vcp, maxdepth=vmaxdepth, minsplit=vminsplit, minbucket=round(vminsplit/vminbucket))
          t1 = Sys.time()
          tiempos[s] <-  as.numeric(  t1 - t0, units = "secs" )
          
          
          #guardamos el modelo
          vcp_and_seed_str <-paste("SEED_", as.character(s),"-CP_", as.character(s),"-DEPTH_", as.character(vmaxdepth),"-SPLIT_", as.character(vminsplit),"-BUCKET_", as.character(vminbucket),  sep = '')
          ...
          ganancias[s] <- ganancia( abril_testing_prediccion,  ds_cliente_premium_testing$clase ) / 0.30
          
          
          #imprimimos salida por pantalla
          print('')
          output <- paste("N: ", n, "SEED:",s, "CP: ", vcp, "DEPTH:", as.character(vmaxdepth), "SPLIT:",vminsplit, "BUCKET:", vminbucket,"Tiempo:", as.numeric(  t1 - t0, units = "secs" ), sep = ' ')
          print(output)
          print('')
          ...
          cat( format(Sys.time(), "%Y%m%d %H%M%S"), "prodprem_201604", "rpart",
               vcp , vminsplit, vminbucket, vmaxdepth, mean(ganancias), mean(tiempos), ganancias , "\n", sep="\t", file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_bestparams", fill=FALSE, append=TRUE )
