...
cat( "fecha", "archivo", "algoritmo", "cp" , "minsplit", "minbucket", "maxdepth", "ganancia_promedio", "tiempo_promedio", "ganancias" , "\n", sep="\t", 
        file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_vardepth", fill=FALSE, append=FALSE )
...
for( vmaxdepth  in  depth_values )
{
  ...
  for( s in  1:5 )
  {
    ...
    t0 =  Sys.time()
    tree_model  <- rpart( clase ~ .   ,   data = ds_cliente_premium_training,   cp=vcp, maxdepth=vmaxdepth)  
    t1 = Sys.time()
    tiempos[s] <-  as.numeric(  t1 - t0, units = "secs" )
    
    
    #guardamos el modelo
    vcp_and_seed_str <-paste("CP_", as.character(vcp),"-SEED_", as.character(s),"-DEPTH_", as.character(vmaxdepth), sep = '')
    ...
    print('')
    output <- paste("Seed: ",s, "CP: ", vcp, "DEPTH: ", vmaxdepth, "Tiempo: ", as.numeric(  t1 - t0, units = "secs" ), sep = ' ')
    print(output)
    print('')

    cat( format(Sys.time(), "%Y%m%d %H%M%S"), "prodprem_201604", "rpart",
       vcp , vminsplit, vminbucket, vmaxdepth, mean(ganancias), mean(tiempos), ganancias , "\n", sep="\t", file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_vardepth", fill=FALSE, append=TRUE )
