...
split_values <- c(20,50,100,200,500,800,1000,1500,2000,2500,3000)
minbucket_values <- c(2,3,4,5)
vcp <- 0 ;
vminbucket <- 8
...
cat( "fecha", "archivo", "algoritmo", "cp" , "minsplit", "minbucket", "maxdepth", "ganancia_promedio", "tiempo_promedio", "ganancias" , "\n", sep="\t", 
        file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_varminbacket", fill=FALSE, append=FALSE )
...
for( vminsplit  in  split_values )
{
  for(vminbucket in minbucket_values)
  {
    ...
    for( s in  1:5 )
    {
      ...
      t0 =  Sys.time()
      tree_model  <- rpart( clase ~ .   ,   data = ds_cliente_premium_training,   cp=vcp, minsplit=vminsplit, minbucket=round(vminsplit/vminbucket))
      t1 = Sys.time()
      tiempos[s] <-  as.numeric(  t1 - t0, units = "secs" )
      
      
      #guardamos el modelo
      vcp_and_seed_str <-paste("CP_", as.character(vcp),"-SEED_", as.character(s),"-SPLIT_", as.character(vminsplit),"-BUCKET_", as.character(vminbucket),  sep = '')
      ...
      print('')
      output <- paste("Seed:",s, "CP:", vcp, "SPlit:",vminsplit, "BUCKET:", vminbucket,"Tiempo:", as.numeric(  t1 - t0, units = "secs" ), sep = ' ')
      print(output)
      print('')
      
      cat( format(Sys.time(), "%Y%m%d %H%M%S"), "prodprem_201604", "rpart",
           vcp , vminsplit, vminbucket, vmaxdepth, mean(ganancias), mean(tiempos), ganancias , "\n", sep="\t", file="/home/guille/Documents/master/datamining_economia/R_projects/results/salida_varminbacket", fill=FALSE, append=TRUE )
